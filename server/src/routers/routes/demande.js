...
router.get('/valid/:cin', (req, res) => {
  (async function getValidation() { // okay , this is complicated XD , the validate() returns a promise
    if (!req.params.cin) {         // we gotta wait for the promise, but when we send directly the returned value
      res.status(422);            // we are sending a promise.. so we need to make another promise that waits for
      res.json({                  // the promise in the validate function .. XD adheka sha3malt lenna 7asilou
        err:'invalid input'
      })    
    } else {
